...
def render_single_stock_panel(mobile_mode: bool = False) -> None:
    st.markdown("### 单只股票分析")
    st.markdown(
        '<div class="caption-box">输入两个信息即可：股票名称（或6位代码） + 买入日期。买入日如遇休市，会自动顺延到下一个交易日。</div>',
        unsafe_allow_html=True,
    )
    ...
    if source_name:
        st.success(f"分析完成：{stock['name']} ({stock['code']})，数据源：{source_name}")
    else:
...
def render_batch_panel(mobile_mode: bool = False) -> None:
    ...
    st.markdown(
        '<div class="caption-box">导入你关心的股票列表后，自动计算 5日/20日/120日/截至最新收益，并汇总胜率、均值和排名。</div>',
        unsafe_allow_html=True,
    )
    ...
    if mobile_mode:
        ...
        k4.metric(
            "截至最新最佳",
            f"{best_row['股票名称']} {best_row['截至最新收益(%)']:.2f}%",
        )
    else:
    ...
    st.caption("Top 10 为截至最新收益最高的10只；Bottom 10 为截至最新收益最低的10只。")
    ...
    csv_bytes = sort_df.to_csv(index=False).encode("utf-8-sig")
    st.download_button(
        "下载结果CSV",
        data=csv_bytes,
        file_name="a_share_batch_result.csv",
        mime="text/csv",
    )
